...
const Login = () => {
  ...
  const handleLoginForm = async (event) => {
    ...
    try {
      await axios.post( 'http://192.168.10.21:8000/api/login',{ email, password },
          {
            headers: {
  ...
  return (
    <div className='w-full bg-gray-900 max-w-xs mx-auto justify-center items-center mt-5'>
      <h2 className='text-center text-white pt-4 font-bold'>
        {' '}
        Login to News App{' '}
      </h2>
      <form onSubmit={handleLoginForm} className='bg-gray shadow-md rounded px-8 pt-6 pb-3 mb-2'>
        <div className='mb-4'>
          <label className='block text-white text-sm font-bold mb-2' htmlFor='username'> Email </label>
      ...
      <Link
        className='inline-block align-baseline p-4 mb-3 ms-4 font-bold text-sm text-blue-500 hover:text-blue-800'
        to='/register' >
